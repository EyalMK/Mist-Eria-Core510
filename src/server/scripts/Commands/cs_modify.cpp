...
public:
	...
	static bool HandleModifyChiCommand(ChatHandler* handler, const char* args)
    {
        ...
        int32 chi = atoi((char*)args)*10;
        int32 chim = atoi((char*)args)*10;
        ...
        handler->PSendSysMessage(LANG_YOU_CHANGE_CHI, handler->GetNameLink(target).c_str(), chi/10, chim/10);
        if (handler->needReportToTarget(target))
            ChatHandler(target->GetSession()).PSendSysMessage(LANG_YOURS_CHI_CHANGED, handler->GetNameLink().c_str(), chi/10, chim/10);

        target->SetMaxPower(POWER_CHI, chim);
        target->SetPower(POWER_CHI, chi);
