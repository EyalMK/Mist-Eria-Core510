    ...
    struct instance_gate_setting_sun_InstanceMapScript : public InstanceScript
    {
        ...
        bool SetBossState(uint32 id, EncounterState state)
        {
            ...
            switch (id)
            {
                ...
                {
                    if (state == DONE)
						for (std::list<uint64>::const_iterator i = mantidBombsGUIDs.begin(); i != mantidBombsGUIDs.end(); ++i)
                            if (GameObject* bomb = instance->GetGameObject((*i)))
                ...
                case DATA_RIMOK:
                {
					...
					for (std::list<uint64>::const_iterator i = secondaryDoorGUIDs.begin(); i != secondaryDoorGUIDs.end(); ++i)
                        HandleGameObject((*i), state != DONE);


					for (std::list<uint64>::const_iterator i = rimokAddGenetarorsGUIDs.begin(); i != rimokAddGenetarorsGUIDs.end(); ++i)
                    {
                        if (Creature* generator = instance->GetCreature((*i)))
                        {
                            if (generator->AI())
                            {
                ...
                case DATA_RAIGONN:
                {
					for (std::list<uint64>::const_iterator i = artilleryGUIDs.begin(); i != artilleryGUIDs.end(); ++i)
                    {
                        if (Creature* artillery = instance->GetCreature((*i)))
                        {
                ...
                case DATA_OPEN_FIRST_DOOR:
                {
                    ...
                    if (GameObject* firstDoor = instance->GetGameObject(firstDoorGuid))
                    {
                        ...
                        if (Creature* trigger = firstDoor->SummonTrigger(firstDoor->GetPositionX(), firstDoor->GetPositionY(), firstDoor->GetPositionZ(), 0, 500))
                        {
							...
							for (std::list<Creature*>::const_iterator i = defensorList.begin(); i != defensorList.end(); ++i)
                            {
                                uint8 random = rand() % 2;

                                float posX = random ? 814.0f:  640.0f;
                                float posY = random ? 2102.0f: 2112.0f;
                                (*i)->KnockbackFrom(posX, posY, 25.0f, 20.0f);
                                (*i)->DespawnOrUnsummon(1000);
                            }
                        }
                    }
                    break;
