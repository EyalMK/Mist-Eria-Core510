            ...
            void HandleBeforeCast()
            {
                if (Unit* caster = GetCaster())
                {
                    if (InstanceScript* instance = caster->GetInstanceScript())
                    {
						...
						for (std::list<Creature*>::const_iterator i = stalkerList.begin(); i != stalkerList.end(); ++i)
                            if (!(*i)->HasAura(SPELL_STRAFIND_RUN_DMG))
                                (*i)->CastSpell((*i), SPELL_STRAFIND_RUN_DMG, true);
                    }
                }
            }
