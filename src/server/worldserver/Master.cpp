...
int Master::Run()
{
    ...
    LoginDatabase.DirectPExecute("UPDATE realmlist SET realmflags = (realmflags & ~%u) | %u WHERE id = '%d'", REALM_FLAG_OFFLINE, REALM_FLAG_INVALID, realmID);
    ...
    LoginDatabase.DirectPExecute("UPDATE realmlist SET realmflags = realmflags & ~%u, population = 0 WHERE id = '%u'", REALM_FLAG_INVALID, realmID);
