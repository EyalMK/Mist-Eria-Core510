    ...
    switch (m_spellInfo->Id)
    {
        case 102793:// Ursol's Vortex
        {
            ...
            if (!targetList.empty())
                for (std::list<Unit*>::const_iterator i = targetList.begin(); i != targetList.end(); ++i)
                    if (!(*i)->HasAura(127797))
                        caster->CastSpell((*i), 127797, true);
        ...
        case 115460:// Healing Sphere
        {
            ...
            if (!targetList.empty())
            {
                for (std::list<Unit*>::const_iterator i = targetList.begin(); i != targetList.end(); ++i)
                {
                    caster->CastSpell((*i), 115464, true); // Healing Sphere heal
                    SetDuration(0);
                    return;
                }
            }
        ...
        case 116011:// Rune of Power
        {
            ...
            if (!targetList.empty())
            {
				for (std::list<Unit*>::const_iterator i = targetList.begin(); i != targetList.end(); ++i)
                {
                    if ((*i)->GetGUID() == caster->GetGUID())
                    {
                        caster->CastSpell((*i), 116014, true); // Rune of Power
                        affected = true;
        ...
        case 116235:// Amethyst Pool
        {
            ...
            if (!targetList.empty())
            {
                for (std::list<Unit*>::const_iterator i = targetList.begin(); i != targetList.end(); ++i)
                {
                    ...
                    if ((*i)->GetDistance(this) > 3.5f)
                        (*i)->RemoveAura(130774);
                    else if (!(*i)->HasAura(130774))
                        caster->CastSpell((*i), 130774, true);
                }
            }
        ...
        case 117032:// Healing Sphere (Afterlife)
        {
            ...
            if (!targetList.empty())
            {
                for (std::list<Unit*>::const_iterator i = targetList.begin(); i != targetList.end(); ++i)
                {
                    if ((*i)->GetGUID() == caster->GetGUID())
                    {
                        caster->CastSpell((*i), 125355, true); // Heal for 15% of life
                        SetDuration(0);
                        return;
                    }
                }
        ...
        case 119031:// Gift of the Serpent (Mastery)
        {
            ...
            if (!targetList.empty())
            {
                for (std::list<Unit*>::const_iterator i = targetList.begin(); i != targetList.end(); ++i)
                {
                    caster->CastSpell((*i), 124041, true); // Gift of the Serpent heal
                    SetDuration(0);
                    return;
                }
            }
        ...
        case 121286:// Chi Sphere (Afterlife)
        {
            ...
            if (!targetList.empty())
            {
                for (std::list<Unit*>::const_iterator i = targetList.begin(); i != targetList.end(); ++i)
                {
                    if ((*i)->GetGUID() == caster->GetGUID())
                    {
                        caster->CastSpell((*i), 121283, true); // Restore 1 Chi
                        SetDuration(0);
                        return;
                    }
                }
        ...
        case 121536:// Angelic Feather
        {
            ...
            if (!targetList.empty())
            {
                for (std::list<Unit*>::const_iterator i = targetList.begin(); i != targetList.end(); ++i)
                {
                    caster->CastSpell((*i), 121557, true); // Angelic Feather increase speed
                    SetDuration(0);
                    return;
                }
            }
        ...
        case 122035:// Path of Blossom
        {
            ...
            if (!targetList.empty())
            {
                for (std::list<Unit*>::const_iterator i = targetList.begin(); i != targetList.end(); ++i)
                {
                    caster->CastSpell((*i), 122036, true); // Path of Blossom damage
                    SetDuration(0);
                    return;
                }
            }
        ...
        case 124506:// Gift of the Ox²
        {
            ...
            for (std::list<Unit*>::const_iterator i = targetList.begin(); i != targetList.end(); ++i)
            {
                ...
                caster->CastSpell((*i), 124507, true); // Gift of the Ox - Heal
                SetDuration(0);
                return;
            }
