...
void Player::UpdateDodgePercentage()
{
    ...
    nondiminishing += float(GetTotalAuraModifier(SPELL_AURA_MOD_DODGE_PERCENT)) / 100.f;
    // Dodge from rating
...
void Player::UpdateExpertise(WeaponAttackType attack)
{
    ...
    switch (attack)
    {
        ...
        case OFF_ATTACK:
        {
            SetStatFloatValue(PLAYER_OFFHAND_EXPERTISE, expertise); 
            break;
        }
        default: 
            break;
...
void Player::UpdateManaRegen()
{
	...
	//Haste Auras that add mana regen
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "ratingbonus %f; auramod %d; auramult %f", GetRatingBonusValue(CR_HASTE_SPELL), GetTotalAuraModifier(SPELL_AURA_HASTE_SPELLS), GetTotalAuraMultiplier(SPELL_AURA_HASTE_SPELLS));
