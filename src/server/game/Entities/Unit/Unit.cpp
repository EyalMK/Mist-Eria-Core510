...
void Unit::DealSpellDamage(SpellNonMeleeDamage* damageInfo, bool durabilityLoss)
{
	...
	{
		uint32 newDamage = uint32(damageInfo->damage + (damageInfo->damage * (GetFloatValue(PLAYER_FIELD_PVP_POWER_DAMAGE)/100)));
		CleanDamage cleanDamage(damageInfo->cleanDamage, damageInfo->absorb, BASE_ATTACK, MELEE_HIT_NORMAL);
		DealDamage(victim, newDamage, &cleanDamage, SPELL_DIRECT_DAMAGE, SpellSchoolMask(damageInfo->schoolMask), spellProto, durabilityLoss);
	}
	else
	{
		CleanDamage cleanDamage(damageInfo->cleanDamage, damageInfo->absorb, BASE_ATTACK, MELEE_HIT_NORMAL);
...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
	...
	if (victim && victim->GetTypeId() == TYPEID_PLAYER)
	{
		...
		DealDamage(victim, newDamage, &cleanDamage, DIRECT_DAMAGE, SpellSchoolMask(damageInfo->damageSchoolMask), NULL, durabilityLoss);
	}
	else
	{
		CleanDamage cleanDamage(damageInfo->cleanDamage, damageInfo->absorb, damageInfo->attackType, damageInfo->hitOutCome);
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_SHAMAN:
        {
            ...
            if (dummySpell->SpellFamilyFlags[0] & 0x200000)
            {
                ...
                float fire_onhit = float(CalculatePct(dummySpell->Effects[EFFECT_0].CalcValue(this), 1.0f));
    ...
    switch (dummySpell->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
            switch (dummySpell->Id)
            {
                ...
                case 59906:
                {
                    int32 bp0 = CalculatePct(GetMaxHealth(), dummySpell->Effects[EFFECT_0]. CalcValue(this));
                ...
                *handled = true;
                ...
                for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
                {
                    if (procSpell->Effects[i].Effect == SPELL_EFFECT_ENERGIZE)
                    {
                        ...
                        int32 mana = procSpell->Effects[i].CalcValue(this) * 2;
            ...
            switch (dummySpell->Id)
            {
                ...
                case 31658:
                {
                    *handled = true;
                    ...
                    int32 bp0 = int32(CalculatePct(GetCreateMana(), spInfo->Effects[0].CalcValue(this)));
        ...
        case SPELLFAMILY_WARRIOR:
        {
            switch (dummySpell->Id)
            {
                ...
                case 70844:
                {
                    int32 basepoints0 = CalculatePct(GetMaxHealth(), dummySpell->Effects[EFFECT_1].CalcValue(this));
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
            ...
            case SPELLFAMILY_GENERIC:
                switch (auraSpellInfo->Id)
                {
                    ...
                    case 43820:             // Charm of the Witch Doctor (Amani Charm of the Witch Doctor trinket)
                        ...
                        basepoints0 = victim->GetCreateHealth() * auraSpellInfo->Effects[EFFECT_1].CalcValue(this) / 100;
                        target = victim;
                        break;
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        case 63158:
            ...
            if (!victim || !victim->HealthBelowPct(auraSpellInfo->Effects[EFFECT_1].CalcValue(this)))
                return false;
            break;
...
void Unit::Kill(Unit* victim, bool durabilityLoss)
{
    ...
    if (isRewardAllowed && player && player != victim)
    {
        ...
        if (creature)
        {
            ...
            loot->generateMoneyLoot(creature->GetCreatureTemplate()->mingold, creature->GetCreatureTemplate()->maxgold, player);
        }
        ...
        switch (element)
        {
            ...
            case MSECounter:
                for (uint32 j = 0; j < counter; ++j)
                    data.read<uint32>(); //?? right ? not test
                break;
            case MSEIsAlive:
...
void Unit::WriteMovementInfo(WorldPacket& data)
{
    ...
    for (uint32 i = 0; i < MSE_COUNT; ++i)
    {
        ...
        case MSEMovementFlags2:
            if (hasMovementFlags2)
                data.WriteBits(mover->GetExtraUnitMovementFlags(), 13);
            break;
        case MSETimestamp:
            if (hasTimestamp)
                data << getMSTime();
        ...
        case MSEAlive:
            if (isAlive)
                data << mover->m_movementInfo.time;
            break;
        case MSEZeroBit:
            data.WriteBit(0);
            break;
