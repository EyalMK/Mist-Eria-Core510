...
int32 Player::GetSpellDamage(uint8 minLevel, uint8 minAttack, uint8 maxLevel, uint8 maxAttack, float attackCoef, float spellCoef, bool heal, WeaponAttackType attackType, SpellSchoolMask schoolMask) const
{
	int32 damage;
...
void Player::InitStatsForLevel(bool reapplyMods)
{
    ...
    float mastery = sMasteryMgr->getMastery(TalentTree(GetPrimaryTalentTree(GetActiveSpec()))).getPercent(0) / 2.0f;
...
float Player::GetSpellCritFromIntellect()
{
    ...
    float intellectFromStuff = GetStat(STAT_INTELLECT) - GetCreateStat(STAT_INTELLECT);

	float ratio = intellectFromStuff / critRatio->ratio;
