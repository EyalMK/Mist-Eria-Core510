...
void AchievementMgr<Player>::SaveToDB(SQLTransaction& trans)
{
    if (!m_completedAchievements.empty())
    {
        ...
        for (CompletedAchievementMap::iterator iter = m_completedAchievements.begin(); iter != m_completedAchievements.end(); ++iter)
        {
            ...
            AchievementEntry const* hf = sAchievementMgr->GetAchievement(iter->first);
            if (hf->flags & ACHIEVEMENT_FLAG_ACCOUNT_WIDE) {
                PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_ACC_ACHIEVEMENT_BY_ACHIEVEMENT);
                stmt->setUInt16(0, iter->first);
                stmt->setUInt32(1, GetOwner()->GetSession()->GetAccountId());
                trans->Append(stmt);

                stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_ACC_ACHIEVEMENT);
    ...
    if (!m_criteriaProgress.empty())
    {
        for (CriteriaProgressMap::iterator iter = m_criteriaProgress.begin(); iter != m_criteriaProgress.end(); ++iter)
        {
            ...
            AchievementCriteriaEntry const* chf = sAchievementMgr->GetAchievementCriteria(iter->first);
            AchievementEntry const* hf = sAchievementMgr->GetAchievement(chf->achievement);

            if (hf->flags & ACHIEVEMENT_FLAG_ACCOUNT_WIDE) {
                PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_ACC_ACHIEVEMENT_PROGRESS_BY_CRITERIA);
                stmt->setUInt32(0, GetOwner()->GetSession()->GetAccountId());
                stmt->setUInt16(1, iter->first);
                trans->Append(stmt);
                ...
                {
                    stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_ACC_ACHIEVEMENT_PROGRESS);
                    stmt->setUInt32(0, GetOwner()->GetSession()->GetAccountId());
                    stmt->setUInt16(1, iter->first);
