...
#define CHECK(exp) \
    if (!(exp))\
    {\
        sLog->outError(LOG_FILTER_NETWORKIO, "\n\n MOVE SPLINE : MoveSplineInitArgs::Validate: expression '%s' failed for GUID: %u Entry: %u \n\n", #exp, unit->GetTypeId() == TYPEID_PLAYER ? unit->GetGUIDLow() : unit->ToCreature()->GetDBTableGUIDLow(), unit->GetEntry());\
        return false;\
    }
    CHECK(path.size() > 1);
    CHECK(velocity > 0.f);
