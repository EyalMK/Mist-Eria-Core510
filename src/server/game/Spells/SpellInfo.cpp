...
int32 SpellEffectInfo::CalcValue(Unit const* caster, int32 const* bp, Unit const* target) const
{
    ...
    sLog->outDebug(LOG_FILTER_SERVER_LOADING, "Pexirn : dont understand why damage works on spell : basePoints : %u", basePoints);
    ...
    sLog->outDebug(LOG_FILTER_SERVER_LOADING, "Pexirn : dont understand why damage works on spell : result : %u", int32(value));
    return int32(value);
}
...
SpellCastResult SpellInfo::CheckTarget(Unit const* caster, WorldObject const* target, bool implicit) const
{
    ...
    //SpellDifficulty not exist in MOP anymore
