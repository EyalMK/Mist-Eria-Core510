...
_flags(AFLAG_NONE), _effctsMask(0), _effectsToApply(effMask), _needClientUpdate(false)
{
...
void AuraApplication::_InitFlags(Unit* caster, uint32 effMask)
{
    ...
    if (IsSelfcasted() || !caster || !caster->IsFriendlyTo(GetTarget()))
    {
        ...
        {
            if (((uint32(1)<<i) & effMask) && !GetBase()->GetSpellInfo()->IsPositiveEffect(i))
            {
                negativeFound = true;
                break;
            }
...
void AuraApplication::_HandleEffect(uint8 effIndex, bool apply)
{
    ...
    ASSERT((uint32(1)<<effIndex) & _effectsToApply);
    ...
    if (apply)
    {
        ASSERT(!(_effctsMask & (uint32(1)<<effIndex)));
        _effctsMask |= 1<<effIndex;
...
void AuraApplication::BuildUpdatePacket(ByteBuffer& data, bool remove) const
{
    ...
    if (flags & AFLAG_ANY_EFFECT_AMOUNT_SENT)
    {
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            if (_effctsMask & (1 << i)) // Effect must be applied on this target
            {
                if (AuraEffect const* eff = aura->GetEffect(i)) // NULL if effect flag not set
                {
                    ...
                    data << float(0); // Prevent error (here should be assert)
            }
        }
        data.put<uint8>(countPos, effectCount);
    }
}
...
{
    if (m_spellInfo->GetPowerPerSecond(caster))
...
void Aura::Update(uint32 diff, Unit* caster)
{
    if (m_duration > 0)
    {
        ...
        if (m_timeCla)
        {
            ...
            {
                if (int32 manaPerSecond = m_spellInfo->GetPowerPerSecond(caster))
                {
                    m_timeCla += 1000 - diff;

                    Powers powertype = Powers(m_spellInfo->GetPowerType(caster));
...
void Aura::RefreshDuration()
{
    ...
    if (m_spellInfo->GetPowerPerSecond(GetCaster()))
        m_timeCla = 1 * IN_MILLISECONDS;
}
