...
void AuraEffect::HandleModInvisibility(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    {
        // apply glow vision
        //if (target->GetTypeId() == TYPEID_PLAYER) //MOP need fix
...
void AuraEffect::HandleAuraOverrideSpells(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        //target->SetUInt16Value(PLAYER_FIELD_BYTES2, 0, overrideId); //MOP need fix
    ...
    {
        //target->SetUInt16Value(PLAYER_FIELD_BYTES2, 0, 0);//MOP need fix
        if (OverrideSpellDataEntry const* overrideSpells = sOverrideSpellDataStore.LookupEntry(overrideId))
            for (uint8 i = 0; i < MAX_OVERRIDE_SPELL; ++i)
                if (uint32 spellId = overrideSpells->spellId[i])
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
    ...
    {
        uint32 funnelDamage = GetSpellInfo()->GetPowerPerSecond(caster); // damage is not affected by spell power
...
void AuraEffect::HandlePeriodicManaLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    if (m_spellInfo->GetPowerCostPercentage(caster))
    {
        // max value
