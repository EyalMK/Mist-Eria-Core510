...
m_spellInfo(info/*sSpellMgr->GetSpellForDifficultyFromSpell(info, caster)*/),/*Mop del spell difficult. may need refix*/
...
void Spell::TakePower()
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER)
    {
        if (powerType == POWER_RAGE || powerType == POWER_ENERGY || powerType == POWER_RUNES || powerType == POWER_CHI || powerType == POWER_CHAOS_ORB)
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (!(_triggeredCastFlags & TRIGGERED_IGNORE_CASTER_AURASTATE))
    {
        ...
        // Spell difficulty not in MOP dbc
                    ...
                    if (msg != EQUIP_ERR_OK)
                    {
                        ...
                        else
                        {
                            if (!(m_spellInfo->SpellFamilyName == SPELLFAMILY_MAGE && (m_spellInfo->SpellFamilyFlags[0] & 0x40000000)))
                                ...
                                p_caster->CastSpell(m_caster, m_spellInfo->Effects[EFFECT_1].CalcValue(p_caster), false);        // move this to anywhere
                            return SPELL_FAILED_DONT_REPORT;
                        }
                    }
                }
