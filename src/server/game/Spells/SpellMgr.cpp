...
SpellInfo const* SpellMgr::GetSpellInfo(uint32 spellId, Unit* caster) const
{
	...
	if(caster)
	{
		if (caster->GetMap())
		{
			const SpellInfo *spellInfo = mSpellInfoMap[caster->GetMap()->GetDifficulty()][spellId];
			if(spellInfo)
				return spellInfo;
		}
	}
...
void SpellMgr::LoadSpellRanks()
{
    ...
    do
    {
        ...
        do
        {
            ...
            mSpellInfoMap[0][addedSpell]->ChainEntry = &mSpellChains[addedSpell];
            prevRank = addedSpell;
            ++itr;
...
void SpellMgr::LoadSpellInfoStore()
{
    ...
    for (uint32 i = 0; i < sSpellEffectStore.GetNumRows(); ++i)
    {
        ...
        effectsBySpell[effect->difficultyMode][effect->EffectSpellId].effects[(effect->EffectIndex)] = effect;
    }
	...
	for (uint8 d = 0; d < MAX_DIFFICULTY; ++d)
	{
		for (uint32 i = 0; i < sSpellStore.GetNumRows(); ++i)
			if (SpellEntry const* spellEntry = sSpellStore.LookupEntry(i))
				if(effectsBySpell[d][i].HasEffects())
					mSpellInfoMap[d][i] = new SpellInfo(spellEntry, effectsBySpell[d][i].effects);
				else
					mSpellInfoMap[d][i] = NULL;
    ...
    for (uint32 i = 0 ; i < sSpellPowerStore.GetNumRows() ; ++i)
    {
		for (uint32 d = 0 ; d < MAX_DIFFICULTY ; ++d)
		{
			...
			SpellInfo * spell = mSpellInfoMap[d][power->spellId];
...
void SpellMgr::UnloadSpellInfoStore()
{
	for (uint32 d = 0; d < MAX_DIFFICULTY; ++d)
	{
		for (uint32 i = 0; i < mSpellInfoMap[0].size(); ++i) // Max spells ids on Difficulty 0
			if (mSpellInfoMap[d][i])
				delete mSpellInfoMap[d][i];
		mSpellInfoMap[d].clear();
	}
}
...
void SpellMgr::UnloadSpellInfoImplicitTargetConditionLists()
{
	for (uint32 d = 0; d < MAX_DIFFICULTY; ++d)
	{
		for (uint32 i = 0; i < mSpellInfoMap[0].size(); ++i)
		{
			if (mSpellInfoMap[d][i])
				mSpellInfoMap[d][i]->_UnloadImplicitTargetConditionLists();
		}
	}
}
...
void SpellMgr::LoadSpellInfoCorrections()
{
	...
	for (uint32 d = 0; d < MAX_DIFFICULTY; ++d)
	{
		...
		for (uint32 i = 0; i < mSpellInfoMap[0].size(); ++i)
		{
			spellInfo = (SpellInfo*)mSpellInfoMap[d][i];
			if (!spellInfo)
				continue;
