...
void ChatHandler::FillMessageData(WorldPacket* data, WorldSession* session, uint8 type, uint32 language, const char *channelName, uint64 target_guid, const char *message, Unit* speaker, const char* addonPrefix /*= NULL*/)
{
    ...
    *data << uint32(CHAT_MSG_PARTY || CHAT_MSG_PARTY_LEADER || CHAT_MSG_RAID);                                     // can be chat msg group or something
...
uint32 ChatHandler::extractSpellIdFromLink(char* text)
{
    ...
    switch (type)
    {
        ...
        case SPELL_LINK_TALENT:
        {
            ...
            sLog->outError(LOG_FILTER_GENERAL, "ChatHandler::extractSpellIdFromLink: Talent not use rank any more ,need fix it");
			
            return 0;//talentEntry->RankID[rank];
        }
