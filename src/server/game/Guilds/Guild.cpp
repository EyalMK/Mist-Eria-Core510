...
void Guild::HandleSetNewGuildMaster(WorldSession* session, std::string const& name)
{
    ...
    else if (Member* oldGuildMaster = GetMember(player->GetGUID()))
    {
        ...
        if (Member* newGuildMaster = GetMember(name))
        {
            ...
            oldGuildMaster->ChangeRank(GR_OFFICER);
...
void Guild::_SendBankContentUpdate(uint8 tabId, SlotIds slots) const
{
    ...
    if (BankTab const* tab = GetBankTab(tabId))
    {
        ...
        {
            if (Item const* tabItem = tab->GetItem(*itr))
            {
                uint32 enchants = 0;
    ...
    if (BankTab const* tab = GetBankTab(tabId))
    {
        for (SlotIds::const_iterator itr = slots.begin(); itr != slots.end(); ++itr)
        {
			...
			data << uint32(tabItem ? tabItem->GetEntry() : 0);
			data << uint32(tabItem ? abs(tabItem->GetSpellCharges()) : 0); // Spell charges
			data << uint32(0);
			data << uint32(tabItem ? tabItem->GetItemRandomPropertyId() : 0);
			data << uint32(tabItem ? tabItem->GetCount() : 0);
			data << uint32(0);
...
void Guild::SendGuildXP(WorldSession* session /* = NULL */) const
{
    ...
    data << uint64(100); //data << uint64(GetTodayExperience());
    data << uint64(GetExperience());
    data << uint64(203);//data << uint64(/*member ? member->GetWeeklyActivity() :*/ 0);
    session->SendPacket(&data);
}
