...
void BattlegroundTK::EventPlayerDroppedOrb(Player* Source, uint32 orb)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
    {
        SetOrbPicker(0, orb);
...
void BattlegroundTK::RemovePlayer(Player* player, uint64 guid, uint32 /*team*/)
{
    ...
    if (m_orbOwners[BG_TK_OBJECT_ORB_BLUE] == guid)
    {
        if (!player)
        {
            ...
            EventPlayerDroppedOrb(player, BG_TK_OBJECT_ORB_BLUE);
    }
    ...
    if (m_orbOwners[BG_TK_OBJECT_ORB_PURPLE] == guid)
    {
        if (!player)
        {
            ...
            EventPlayerDroppedOrb(player, BG_TK_OBJECT_ORB_PURPLE);
    }
	...
	if (m_orbOwners[BG_TK_OBJECT_ORB_GREEN] == guid)
    {
        if (!player)
        {
            ...
            EventPlayerDroppedOrb(player, BG_TK_OBJECT_ORB_GREEN);
    }
	...
	if (m_orbOwners[BG_TK_OBJECT_ORB_ORANGE] == guid)
    {
        if (!player)
        {
            ...
            EventPlayerDroppedOrb(player, BG_TK_OBJECT_ORB_ORANGE);
    }
}
...
bool BattlegroundTK::SetupBattleground()
{
    ...
    if (!AddObject(BG_TK_OBJECT_ORB_BLUE, BG_OBJECT_TK_ORB_BLUE, 1716.808105f, 1249.910645f, 10.675041f, 0.0f, 0, 0, 0.9996573f, 0.02617699f, RESPAWN_IMMEDIATELY)
      ...
      // horde gate
       ...
       // alliance gate
        || !AddObject(BG_TK_OBJECT_DOOR_A, BG_OBJECT_DOOR_TK_A, 1783.735107f, 1104.960205f, 19.374258f, 1.59667f, 0.0f, 0.0f, 0.71615f, 0.697947f, RESPAWN_IMMEDIATELY)
		)
    {
    ...
    WorldSafeLocsEntry const* sg = sWorldSafeLocsStore.LookupEntry(TK_GRAVEYARD_MAIN_ALLIANCE);
    if (!sg || !AddSpiritGuide(TK_SPIRIT_MAIN_ALLIANCE, sg->x, sg->y, sg->z, 3.124139f, ALLIANCE))
    {
        sLog->outError(LOG_FILTER_SQL, "BattleGroundTK: Failed to spawn Alliance spirit guide! Battleground not created!");
        return false;
    }

    sg = sWorldSafeLocsStore.LookupEntry(TK_GRAVEYARD_MAIN_HORDE);
    if (!sg || !AddSpiritGuide(TK_SPIRIT_MAIN_HORDE, sg->x, sg->y, sg->z, 3.193953f, HORDE))
    {
...
{
	if(m_orbOwners[0 || 1 || 2 || 3] == player->GetGUID())
		return true;
	else
		return false;
}
