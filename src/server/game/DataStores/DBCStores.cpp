...
DBCStorage <SpecializationSpellEntry> sSpecializationSpellStorage(SpecializationSpellfmt);
...
inline void LoadDBC(uint32& availableDbcLocales, StoreProblemList& errors, DBCStorage<T>& storage, std::string const& dbcPath, std::string const& filename, std::string const* customFormat = NULL, std::string const* customIndexName = NULL)
{
    ...
    SqlDbc * sql = NULL;
    if (customFormat)
        sql = new SqlDbc(&filename, new std::string(storage.GetFormat()), customIndexName, storage.GetFormat());
...
void LoadDBCStores(const std::string& dataPath)
{
   ...
   // LoadDBC(availableDbcLocales, bad_dbc_files, sNumTalentsAtLevelStore,      dbcPath, "NumTalentsAtLevel.dbc");//14545
	...
	LoadDBC(availableDbcLocales, bad_dbc_files, sSpecializationSpellStorage,  dbcPath, "SpecializationSpells.dbc");
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellMiscStore,              dbcPath,"SpellMisc.dbc", &CustomSpellMiscEntryfmt, &CustomSpellMiscEntryIndex);
    ...
    LoadDBC(availableDbcLocales, bad_dbc_files, sSpellEffectScalingStore,     dbcPath,"SpellEffectScaling.dbc");//14545
...
{
    return NULL;//sTalentTabPages[cls];  //MOP talent changed
}

std::vector<uint32> const* GetTalentTreePrimarySpells(uint32 talentTree)
{
    return NULL;//MOP talent changed
